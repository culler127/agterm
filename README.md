...
- Node.js ≥ 20.19 or ≥ 22.7 on `PATH` for the OpenCode status-plugin unit tests (`OpenCodeStatusHookTests` spawns it; those versions unflag module-syntax detection for the plugin's bare `.js`). The app itself does not need Node at runtime; without a qualifying Node those tests skip.
