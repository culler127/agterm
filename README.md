...
- Node.js on `PATH` for the OpenCode status-plugin unit tests (`OpenCodeStatusHookTests` spawns it). The app itself does not need Node at runtime; without Node those tests skip.
...
For OpenCode, the installer copies a bundled JavaScript lifecycle plugin to `~/.config/opencode/plugins/agterm-status.js` when OpenCode has already created `~/.config/opencode`. The file exports only the plugin function (OpenCode's legacy loader treats every export as a plugin). OpenCode `session.status` `busy`/`retry` set `active --blink`; `idle` sets `completed --auto-reset` only for a sessionID that previously saw busy/retry (so a subagent's idle cannot paint completed onto a still-working parent). Permission/question prompts set `blocked`; `session.error` also sets `blocked` and suppresses the following `session.status(idle)` that OpenCode's halt path always publishes, so a failed turn stays blocked rather than flipping to completed. Reply/reject events clear blocked with `active --blink`. Deprecated `session.idle` is ignored so it does not double-fire with `session.status(type=idle)`. It preserves a same-named plugin without agterm's ownership marker; restart OpenCode after installing or upgrading it.
